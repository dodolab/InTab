...
namespace InteractiveTable.GUI.CaptureSet
{
    public partial class TemplateViewer : Form
    {
        ...
        public TemplateViewer(Templates templates, Templates samples, IImage image)
        {
            if (image == null)
                return;
            InitializeComponent();
        ...
        //        e.Graphics.DrawImage(bmp, boundRect, new Rectangle(new Point(boundRect.X, boundRect.Y), new Size(50,50)),
            //        GraphicsUnit.Pixel);
            }
        ...
        {
            if (tbTemplateName.Text == "<jméno šablony>")
                MessageBox.Show("Vložte jméno šablony");
            else
                try
                {
